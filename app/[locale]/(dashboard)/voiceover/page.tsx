...
const VOICES = [
  { id: 'male_pro', label: 'رجل - احترافي' },
  { id: 'female_pro', label: 'امرأة - احترافية' },
  { id: 'male_youth', label: 'رجل - شبابي' },
  { id: 'female_youth', label: 'امرأة - شبابية' },
  { id: 'male_formal', label: 'رجل - رسمي' },
] as const;

const DIALECTS = ['saudi', 'emirati', 'egyptian', 'gulf', 'formal'] as const;
const SPEEDS = [{ id: '0.75', label: 'بطيء' }, { id: '1', label: 'عادي' }, { id: '1.25', label: 'سريع' }] as const;
...
export default function VoiceOverPage(): React.ReactElement {
  ...
  const dialectLabels: Record<string, string> = { saudi: 'سعودية', emirati: 'إماراتية', egyptian: 'مصرية', gulf: 'خليجية', formal: 'فصحى' };
  const toneLabels: Record<string, string> = { professional: 'احترافي', friendly: 'ودود', energetic: 'حماسي', calm: 'هادئ' };
  ...
  const inputPanel = (
    <div className="space-y-4">
      ...
      <div className="space-y-2"><Label>اللهجة</Label><div className="flex flex-wrap gap-2">{DIALECTS.map((d) => (<button key={d} type="button" onClick={() => setDialect(d)} className={cn('rounded-full px-3 py-1.5 text-xs transition-colors', dialect === d ? 'bg-primary-500 text-white' : 'bg-surface-2')}>{dialectLabels[d]}</button>))}</div></div>
      <div className="space-y-2"><Label>السرعة</Label><div className="flex gap-2">{SPEEDS.map((s) => (<button key={s.id} type="button" onClick={() => setSpeed(s.id)} className={cn('flex-1 rounded-lg border px-3 py-2 text-xs transition-colors', speed === s.id ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-[var(--color-border)] hover:border-primary-300')}>{s.label}</button>))}</div></div>
      <div className="space-y-2"><Label>النبرة</Label><div className="flex flex-wrap gap-2">{TONES.map((tn) => (<button key={tn} type="button" onClick={() => setTone(tn)} className={cn('rounded-full px-3 py-1.5 text-xs transition-colors', tone === tn ? 'bg-primary-500 text-white' : 'bg-surface-2')}>{toneLabels[tn]}</button>))}</div></div>
  ...
  const previewPanel = isLoading ? (
    ...
    <div className="flex flex-col items-center py-12 text-[var(--color-text-muted)]"><Mic className="h-12 w-12" /><p className="text-sm mt-4">التعليق الصوتي سيظهر هنا</p></div>
  ) : (
    <div className="flex flex-col items-center py-12 gap-6">
      ...
      <div className="w-full max-w-sm bg-surface-2 rounded-xl p-6 flex flex-col items-center gap-4">
        ...
        <p className="text-xs text-[var(--color-text-muted)]">{audioDuration} ثانية</p>
      </div>
      <Button variant="outline" className="gap-2"><Download className="h-4 w-4" />تحميل MP3</Button>
    </div>
  );

  return (
    <div className="h-[calc(100vh-3.5rem)]">
      <div className="px-6 py-4 border-b"><h1 className="text-xl font-bold font-cairo">{t('nav.voiceover')}</h1><p className="text-sm text-[var(--color-text-secondary)]">توليد تعليق صوتي احترافي</p></div>
