...
const EDIT_TYPES = [
  { id: 'background_replace', label: 'تغيير الخلفية', emoji: '🖼️' },
  { id: 'object_remove', label: 'إزالة عنصر', emoji: '🗑️' },
  { id: 'color_change', label: 'تغيير اللون', emoji: '🎨' },
  { id: 'text_add', label: 'إضافة نص', emoji: '✍️' },
  { id: 'style_transfer', label: 'تغيير الأسلوب', emoji: '🔄' },
] as const;
...
export default function EditPage(): React.ReactElement {
      ...
      <div className="space-y-2">
        ...
        {originalImage ? (
          ...
          <button type="button" onClick={() => fileRef.current?.click()} className="flex flex-col items-center gap-2 w-full rounded-lg border-2 border-dashed border-[var(--color-border)] p-8 hover:border-primary-300 transition-colors">
            <Upload className="h-8 w-8 text-[var(--color-text-muted)]" /><span className="text-sm text-[var(--color-text-muted)]">ارفع الصورة</span>
          </button>
        )}
        <input ref={fileRef} type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) setOriginalImage(URL.createObjectURL(f)); }} className="hidden" />
      </div>
      <div className="space-y-2">
  ...
  const previewPanel = isLoading ? (
    ...
    <div className="flex flex-col items-center py-12 text-[var(--color-text-muted)]"><span className="text-5xl">✏️</span><p className="text-sm mt-4">النتيجة ستظهر هنا</p></div>
  ) : (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div><p className="text-xs font-medium mb-2 text-center">الأصلية</p>{originalImage && /* eslint-disable-next-line @next/next/no-img-element */ <img src={originalImage} alt="Original" className="w-full rounded-lg border" />}</div>
        <div><p className="text-xs font-medium mb-2 text-center">بعد التعديل</p>{resultImage ? /* eslint-disable-next-line @next/next/no-img-element */ <img src={resultImage} alt="Edited" className="w-full rounded-lg border" /> : <div className="w-full aspect-square rounded-lg border-2 border-dashed border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] text-sm">اضغط توليد</div>}</div>
      </div>
  ...
  return (
    <div className="h-[calc(100vh-3.5rem)]">
      <div className="px-6 py-4 border-b"><h1 className="text-xl font-bold font-cairo">{t('nav.edit')}</h1><p className="text-sm text-[var(--color-text-secondary)]">عدّل صورك بالذكاء الاصطناعي</p></div>
