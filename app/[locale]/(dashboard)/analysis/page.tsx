...
export default function AnalysisPage(): React.ReactElement {
  ...
  const inputPanel = (
    <div className="space-y-4">
      ...
      <div className="space-y-2"><Label htmlFor="analysis-description">{tAn('businessDescription')}</Label><textarea id="analysis-description" value={description} onChange={(e) => setDescription(e.target.value)} onKeyDown={handleSubmitKeyDown} placeholder={tAn('descriptionPlaceholder')} rows={3} maxLength={2000} className="flex w-full rounded-md border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2 text-sm placeholder:text-[var(--color-text-muted)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 resize-none" /><p className="text-xs text-end text-[var(--color-text-muted)]">{description.length}/2000</p></div>
