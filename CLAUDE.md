...
| `photoshoot` and `storyboard` read brand context only from a **project**, so P4.2's restaurant→`food` default was unreachable on the exact journey this branch was built for | ⚠️ **fixed in the CLIENT only — corrected 2026-09-01.** `ff239bf` is a two-file diff (`storyboard/page.tsx`, `PhotoshootForm.tsx`); both now send `projectKit ?? defaultKit`. **Neither route changed.** `photoshoot/route.ts:88-98` and `storyboard/route.ts:80-100` are still `if (input.brandKitId)` and nothing else — neither file contains `is_default`, `from('projects')` or `brand_kit_id`. So the fallback exists only while the request comes from that exact page: `scripts/live/` never sends `brandKitId`, and six of the seven kit-capable studios have therefore **never** had their kit resolution exercised by any verification |
